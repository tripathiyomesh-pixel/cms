...
export default function PDPF5Split({ product, related, goldRate, waNumber }) {
  ...
  return (
    <div style={{ background:'var(--color-bg)',minHeight:'100vh' }}>
      ...
      <div style={{ display:'flex',height:'100vh',minHeight:600 }}>
        {/* Left: image */}
        <div style={{ flex:'0 0 50%',overflow:'hidden',background:'#f5ede2',position:'relative' }}>
          {images[active] ? (
            <img src={images[active]} alt={product.name} style={{ width:'100%',height:'100%',objectFit:'cover',transition:'opacity 300ms ease' }}/>
          ) : (
            <div style={{ width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:120,fontFamily:'var(--font-heading)',color:G,opacity:0.2 }}>
              {(product.name||'T')[0]}
            </div>
          )}
          {/* Thumbnail dots */}
            ...
            <div style={{ position:'absolute',bottom:24,left:'50%',transform:'translateX(-50%)',display:'flex',gap:8 }}>
              {images.map((_,i) => (
                <button key={i} onClick={()=>setActive(i)}
                  style={{ width:active===i?24:8,height:8,borderRadius:active===i?4:4,background:active===i?G:'rgba(255,255,255,0.5)',border:'none',cursor:'pointer',transition:'all 200ms ease' }}/>
              ))}
            </div>
          )}
        </div>
        ...
        {/* Right: centered info */}
        <div style={{ flex:'0 0 50%',display:'flex',alignItems:'center',justifyContent:'center',padding:'32px 56px',overflowY:'auto' }}>
      ...
      {related?.length > 0 && (
        <div style={{ maxWidth:1400,margin:'0 auto',padding:'64px 24px 80px' }}>
