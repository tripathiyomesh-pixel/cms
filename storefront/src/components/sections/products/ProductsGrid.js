...
export default function ProductsGrid({ config = {}, cols = 4 }) {
  ...
  const bg      = config.featured_bg      || 'var(--color-bg)';
  ...
  return (
    <section style={{ padding:'80px 0', background:bg }}>
      <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 40px' }}>
        <p style={{ fontSize:10,fontWeight:500,letterSpacing:'0.3em',textTransform:'uppercase',color:'var(--color-accent)',textAlign:'center',marginBottom:10 }}>{label}</p>
        <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:44,fontWeight:300,color:'var(--color-text)',textAlign:'center',marginBottom:52 }}>{heading}</h2>
            ...
            return (
              <div key={p.id} className="group" style={{ background:'#fff', border:'1px solid transparent', transition:'border-color .2s' }}
                ...
                <Link href={`/jewellery/${p.slug||p.id}`} className="block relative overflow-hidden" style={{ aspectRatio:'1', background:'#f5ede2' }}>
                  ...
                  {badge && <span style={{ position:'absolute',top:12,left:12,background:'var(--color-text)',color:'#fff',fontSize:9,fontWeight:600,padding:'3px 8px',letterSpacing:'0.08em' }}>{badge}</span>}
                ...
                <div style={{ padding:16 }}>
                  <h3 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:400,color:'var(--color-text)',marginBottom:12,lineHeight:1.3 }}>{p.name}</h3>
                  {(p.base_price||p.final_price) && Number(p.base_price||p.final_price)>0 ? <p style={{ fontSize:13,color:'#6b6b6b',marginBottom:12 }}>{p.currency||'AED'} {Number(p.base_price||p.final_price).toLocaleString()}</p> : <p style={{fontSize:12,color:'var(--color-accent)',marginBottom:12}}>Price on Request</p>}
                  {wapp && <a href={`https://wa.me/${wapp.replace(/\D/g,'')}?text=${msg}`} target="_blank" rel="noreferrer"
                    style={{ display:'block',width:'100%',padding:'12px',background:'var(--color-text)',color:'#fff',fontSize:10,fontWeight:500,letterSpacing:'0.15em',textTransform:'uppercase',textAlign:'center',textDecoration:'none',transition:'background .2s' }}
        ...
        <div style={{ textAlign:'center',marginTop:48 }}>
          <Link href="/jewellery" style={{ display:'inline-block',padding:'13px 40px',border:'1px solid #1a1a1a',color:'var(--color-text)',fontSize:11,fontWeight:500,letterSpacing:'0.15em',textTransform:'uppercase',transition:'all .2s' }}
