  ...
  return (
    <section style={{ padding:'80px 60px', background:bg }}>
      <div style={{ maxWidth:1200,margin:'0 auto' }}>
        <p style={{ fontSize:10,fontWeight:500,letterSpacing:'0.3em',textTransform:'uppercase',color:'#b8860b',textAlign:'center',marginBottom:12 }}>
          {config.testimonials_label||'What Our Clients Say'}
        </p>
        <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:44,fontWeight:300,color:'#1a1a1a',textAlign:'center',marginBottom:56 }}>Client Stories</h2>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:32 }}>
          {REVIEWS.map((r,i)=>(
            <div key={i} style={{ padding:32,border:'1px solid #e5e0d8',background:'#fff' }}>
              <div style={{ color:'#b8860b',fontSize:16,letterSpacing:3,marginBottom:20 }}>{'★'.repeat(r.rating)}</div>
              <p style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:300,fontStyle:'italic',color:'#1a1a1a',lineHeight:1.7,marginBottom:24 }}>"{r.text}"</p>
              <div style={{ borderTop:'1px solid #f0ede8',paddingTop:16 }}>
                <p style={{ fontSize:12,fontWeight:600,color:'#1a1a1a' }}>{r.name}</p>
                <p style={{ fontSize:11,color:'#b8860b',marginTop:2 }}>"{r.title}"</p>
              </div>
            </div>
          ))}
        </div>
